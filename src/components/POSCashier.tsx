...
import { calculateSubtotal, calculateTax, calculateDiscount, calculateTotal, calculateChange } from "../lib/pos-calculator";
...
export const POSCashier: React.FC<POSCashierProps> = ({ token }) => {
  ...
  const addToCart = (product: any) => {
    if (product.stock <= 0) {
      alert(`Stok produk '${product.name}' sedang kosong!`);
      return;
    }
        ...
        .map((item) => {
          if (item.product.id === productId) {
            ...
            if (newQty > item.product.stock) {
              alert(`Stok produk hanya tersisa ${item.product.stock}`);
              return item;
            }
  ...
  const subtotal = calculateSubtotal(cart.map((i) => ({ price: i.unitPrice, quantity: i.quantity })));
  ...
  const handleProcessCheckout = async () => {
    if (paymentAmount < grandTotal && paymentMethod === "CASH") {
      setErrorMessage(`Jumlah pembayaran (Rp ${paymentAmount.toLocaleString("id-ID")}) kurang dari total (Rp ${grandTotal.toLocaleString("id-ID")})`);
      return;
    }
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          {filteredProducts.length === 0 ? (
            <div className="glass-card p-12 text-center text-slate-400 h-full flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-slate-500 mt-1">Coba sesuaikan kata kunci pencarian atau kategori filter.</p>
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`glass-card glass-card-hover p-3.5 flex flex-col justify-between relative group cursor-pointer border ${
                      isOutOfStock ? "opacity-60 cursor-not-allowed border-red-500/20" : "border-slate-800"
                    }`}
                  >
                    <div>
      ...
      <div className="glass-card p-5 flex flex-col justify-between h-full border border-indigo-500/20">
        <div>
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-lg text-white">Keranjang POS</h3>
            </div>
          ...
          <div className="mt-4 space-y-3 max-h-[calc(100vh-420px)] overflow-y-auto pr-1">
            {cart.length === 0 ? (
              <div className="py-12 text-center text-slate-500 text-sm">
                ...
                <p>Klik produk di sebelah kiri untuk menambah ke keranjang</p>
              </div>
            ) : (
              cart.map((item) => (
                <div
                ...
                >
                  <div className="flex-1 pr-2">
                    <h5 className="font-semibold text-xs text-slate-200 line-clamp-1">{item.product.name}</h5>
                    <p className="text-[11px] text-slate-400 mt-0.5">{formatIDR(item.unitPrice)}</p>
                  </div>
        ...
        <div className="pt-4 border-t border-slate-800 space-y-2 text-xs">
          ...
          <div className="flex justify-between text-slate-400">
            ...
            <span className="font-medium text-slate-200">{formatIDR(taxAmount)}</span>
          </div>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-card max-w-md w-full p-6 relative border border-slate-700 shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-white mb-1">Pembayaran Kasir POS</h3>
            <p className="text-xs text-slate-400 mb-4">Pilih metode pembayaran dan masukkan jumlah uang bayar.</p>
            ...
            <div className="space-y-4 text-sm">
              <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800">
                <div className="flex justify-between text-xs text-slate-400">
                  <span>Tagihan Tagihan:</span>
                  <span className="font-bold text-white text-sm">{formatIDR(grandTotal)}</span>
                </div>
              </div>
              ...
              {paymentMethod === "CASH" && (
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1">Jumlah Uang Diterima:</label>
                  <input
                    type="number"
                    value={paymentAmount}
                  ...
                  <div className="mt-3 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex justify-between items-center">
                    ...
                    <span className={`text-base font-extrabold ${changeAmount < 0 ? "text-red-400" : "text-emerald-400"}`}>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white text-slate-900 max-w-sm w-full p-6 rounded-2xl shadow-2xl relative font-mono text-xs">
            ...
            <div className="text-center pb-4 border-b border-dashed border-slate-300">
              ...
              <h3 className="text-base font-extrabold tracking-wider uppercase text-slate-900">POS CAFE & RESTO</h3>
              <p className="text-[10px] text-slate-500">Struk Resmi Pembayaran</p>
              <p className="text-[10px] text-slate-700 font-bold mt-2">{receiptData.invoiceNo}</p>
